...
export const globalErrorHandler = (error: any, req: Request, res: Response, next: NextFunction) => {
	...
	if (error instanceof httpErrors.NotFound) {
		...
		res.status(error.status).json({
			message: hasCustomMessage ? error.message : "No permission to access resource.",
		});
		return;
	} else if (error instanceof httpErrors.BadRequest) {
		res.status(error.status).json({
			message: hasCustomMessage ? error.message : "One or more of the provided values are invalid.",
		});
		return;
	}
